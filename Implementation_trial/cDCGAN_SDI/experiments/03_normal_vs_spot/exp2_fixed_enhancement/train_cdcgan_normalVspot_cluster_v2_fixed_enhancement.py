...
OUT_ROOT = "/shared/rc/defgengan/outputs/exp_spot_2_fixed_enhancement_normal_vs_spot"
...
def build_balanced_file_lists():
    ...
    print("Normal:", balanced_labels.count(0))
    print("Spot:", balanced_labels.count(1))

    return balanced_files, balanced_labels


# =========================================================
# FIXED SPOT ENHANCEMENT
# =========================================================
def enhance_spot_pil_fixed(img_pil):
    """
    Fixed spot enhancement:
    - autocontrast
    ...
    Purpose:
    make local blob-like spot defects more visible
    without adding randomness yet
    """
    img = ImageOps.autocontrast(img_pil)
...
def load_and_preprocess_tf(path, label):
    ...
    try:
        with Image.open(path_str) as pil_img:
            ...
            # Only spot class gets fixed enhancement
            if label_int == 1:
                img = enhance_spot_pil_fixed(img)
...
def build_tf_dataset(file_paths, labels, batch_size=BATCH_SIZE):
    ...
    ds = ds.map(tf_wrapper, num_parallel_calls=AUTOTUNE)
...
def main():
    ...
    with open(os.path.join(LOSS_DIR, "run_summary.json"), "w") as f:
        json.dump({
            ...
            "spot_enhancement": "fixed_autocontrast_plus_contrast",
            "augmentation": False
        }, f, indent=2)
