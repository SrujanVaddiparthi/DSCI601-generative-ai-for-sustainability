...
OUT_ROOT = "/shared/rc/defgengan/outputs/exp_spot_3_stratified_randomized"
...
# =========================================================
# RANDOMIZED SPOT ENHANCEMENT
# =========================================================
def enhance_spot_pil_random(img_pil):
    """
    Spot gets randomized local-contrast enhancement,
    but no geometric augmentation yet.
    """
    img = img_pil

    # Randomized autocontrast + contrast range
    img = ImageOps.autocontrast(img)

    contrast_factor = np.random.uniform(1.2, 1.9)
    img = ImageEnhance.Contrast(img).enhance(contrast_factor)

    # Mild sharpness variation so blobs can stand out a bit
...
def load_and_preprocess_tf(path, label):
    ...
    try:
        with Image.open(path_str) as pil_img:
            ...
            if label_int == 1:
                img = enhance_spot_pil_random(img)
