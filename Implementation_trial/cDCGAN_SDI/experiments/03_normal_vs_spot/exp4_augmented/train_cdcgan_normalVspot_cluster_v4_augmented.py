...
OUT_ROOT = "/shared/rc/defgengan/outputs/exp_spot_4_stratified_randomized_augmented"
...
def build_balanced_class_lists():
    ...
    if len(spot_files) >= TARGET_COUNTS[1]:
        spot_balanced = random.sample(spot_files, TARGET_COUNTS[1])
    else:
        spot_balanced = oversample_to_target(spot_files, TARGET_COUNTS[1])

    print("\nBalanced class counts:")
    print("Normal:", len(normal_balanced))
    print("Spot:", len(spot_balanced))

    return normal_balanced, spot_balanced
...
def augment_normal_pil(img_pil):
    """
    Same philosophy as scratch exp6:
...
def enhance_and_augment_spot_pil(img_pil):
    """
    Spot gets:
    - randomized local contrast enhancement
    - mild sharpness variation
    - mild rotation
    - optional horizontal flip

    Keep it mild so we do not invent artificial spot structure.
    """
    img = img_pil

    img = ImageOps.autocontrast(img)

    contrast_factor = np.random.uniform(1.2, 1.9)
    img = ImageEnhance.Contrast(img).enhance(contrast_factor)

    sharpness_factor = np.random.uniform(1.0, 1.4)
    img = ImageEnhance.Sharpness(img).enhance(sharpness_factor)

    img = mild_rotate(img, max_degrees=10)
    ...
    try:
        with Image.open(path_str) as pil_img:
            ...
            if label_int == 0:
                ...
                img = enhance_and_augment_spot_pil(img)
        ...
        print(f"[ERROR] Failed to load/process image: {path_str} | label={label_int} | error={e}")
        raise
...
# =========================================================
def combine_batches(normal_batch, spot_batch):
    normal_imgs, normal_labels = normal_batch
    spot_imgs, spot_labels = spot_batch

    images = tf.concat([normal_imgs, spot_imgs], axis=0)
    labels = tf.concat([normal_labels, spot_labels], axis=0)
...
def build_stratified_tf_dataset(normal_files, spot_files, batch_size=BATCH_SIZE):
    ...
    spot_ds = tf.data.Dataset.from_tensor_slices((spot_files, spot_labels))
    spot_ds = spot_ds.shuffle(len(spot_files), seed=SEED, reshuffle_each_iteration=True)
    spot_ds = spot_ds.repeat()
    spot_ds = spot_ds.map(tf_wrapper, num_parallel_calls=AUTOTUNE)
    spot_ds = spot_ds.batch(half_batch, drop_remainder=True)

    ds = tf.data.Dataset.zip((normal_ds, spot_ds))
    ds = ds.map(combine_batches, num_parallel_calls=AUTOTUNE)
    ds = ds.prefetch(AUTOTUNE)

    steps_per_epoch = min(len(normal_files) // half_batch, len(spot_files) // half_batch)
...
def main():
    ...
    normal_files, spot_files = build_balanced_class_lists()
    dataset, steps_per_epoch = build_stratified_tf_dataset(normal_files, spot_files, batch_size=BATCH_SIZE)
    ...
    with open(os.path.join(LOSS_DIR, "run_summary.json"), "w") as f:
        json.dump({
            ...
            "spot_enhancement": "randomized_autocontrast_contrast_sharpness",
            "augmentation": True,
            "normal_mild_rotation_only": True,
            "spot_mild_rotation_flip": True
        }, f, indent=2)
