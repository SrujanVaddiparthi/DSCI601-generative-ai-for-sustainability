...
OUT_ROOT = "/shared/rc/defgengan/outputs/exp5_normal_vs_scratch_stratified"
...
def build_balanced_class_lists():
    ...
    # Normal: subsample if above target
    ...
    # Scratch: oversample if below target
...
# =========================================================
# RANDOMIZED SCRATCH ENHANCEMENT
# =========================================================
def enhance_scratch_pil_random(img_pil):
    """
    Randomized enhancement so the model does not memorize one fixed contrast signature.
    """
    img = img_pil

    # Random contrast range
    contrast_factor = np.random.uniform(1.2, 2.0)
    img = ImageEnhance.Contrast(img).enhance(contrast_factor)

    # Random sharpness range
    sharpness_factor = np.random.uniform(1.0, 2.0)
    img = ImageEnhance.Sharpness(img).enhance(sharpness_factor)

    # Optional extra sharpen pass with probability
...
def load_and_preprocess_tf(path, label):
    ...
    # Only scratch class gets randomized enhancement
    if label_int == 1:
        img = enhance_scratch_pil_random(img)
...
def combine_batches(normal_batch, scratch_batch):
    ...
    # Shuffle within the batch so order is not always [all normal, all scratch]
