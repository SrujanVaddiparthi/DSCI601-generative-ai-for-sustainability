...
# Optional output structure; can be changed by cluster workflow later
...
CHECKPOINT_EPOCHS = [19, 20, 21, 38, 39, 40, 41, 42, 43, 44, 45, 79, 80, 81, 82]
...
def build_models() -> tuple[keras.Model, keras.Model, keras.Model]:
    ...
    # Freeze discriminator for GAN graph
...
def train_cdcgan(generator: keras.Model, discriminator: keras.Model, gan_model: keras.Model,
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        # Make D trainable for discriminator updates
        discriminator.trainable = True
        for layer in discriminator.layers:
            layer.trainable = True
        ...
        if epoch in CHECKPOINT_EPOCHS:
            ...
            discriminator.save(os.path.join(CHECKPOINT_DIR, f"discriminator_epoch_{epoch:03d}.keras"))
            gan_model.save(os.path.join(CHECKPOINT_DIR, f"gan_epoch_{epoch:03d}.keras"))
